...
LONG WINAPI UnhandledExceptionHandler(struct _EXCEPTION_POINTERS *ExceptionInfo)
{
    ...
    swprintf_s(dmpFileName, L"%ls\\Selene-%I64u.dmp",
...
int main(int argc, char *argv[])
{
    ...
    // The organization name becomes a directory name, so keep it free of spaces:
    ...
    if (WMUtils::isRunningWindowManager()) {
        ...
        if (!qEnvironmentVariableIsSet("QT_QPA_PLATFORM")) {
            ...
            if (!QFile("/dev/dri").exists()) {
                ...
                qWarning() << "On the Raspberry Pi, you must enable the 'fake KMS' driver in raspi-config to use Selene outside of the GUI environment.";
            }
    ...
    if (!qEnvironmentVariableIsSet("QT_OPENGL")) {
        ...
        // seem to crash Selene far more often than DirectX.
    ...
    app.setWindowIcon(QIcon(":/res/selene.svg"));
#endif
