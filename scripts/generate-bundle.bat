...
cmd /c "set VERSION= && msbuild -Restore %SOURCE_ROOT%\wix\SeleneSetup\SeleneSetup.wixproj /p:Configuration=%BUILD_CONFIG% /p:Platform=x86 /p:MSBuildProjectExtensionsPath=%BUILD_FOLDER%\"
if !ERRORLEVEL! NEQ 0 goto Error

rem Rename the installer to match the publishing convention
ren %INSTALLER_FOLDER%\SeleneSetup.exe SeleneSetup-%VERSION%.exe

echo Build successful for Selene v%VERSION% installer!
exit /b 0
