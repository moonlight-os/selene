...
pushd $BUILD_FOLDER
qmake $SOURCE_ROOT/selene.pro QMAKE_APPLE_DEVICE_ARCHS="x86_64 arm64" TELEMETRY_URL="$TELEMETRY_URL" || fail "Qmake failed!"
popd
...
if [ "$SIGNING_IDENTITY" != "" ]; then
  ...
  codesign --force --deep --options runtime --timestamp --sign "$SIGNING_IDENTITY" $BUILD_FOLDER/app/Selene.app || fail "Signing failed!"
fi

echo Creating DMG
if [ "$SIGNING_IDENTITY" != "" ]; then
  create-dmg $BUILD_FOLDER/app/Selene.app $INSTALLER_FOLDER --identity="$SIGNING_IDENTITY" --no-version-in-filename || fail "create-dmg failed!"
else
  create-dmg $BUILD_FOLDER/app/Selene.app $INSTALLER_FOLDER --no-version-in-filename
  case $? in
    0) ;;
    2) ;;
...
if [ "$NOTARY_KEYCHAIN_PROFILE" != "" ]; then
  ...
  xcrun notarytool submit --keychain-profile "$NOTARY_KEYCHAIN_PROFILE" --wait $INSTALLER_FOLDER/Selene.dmg || fail "Notary submission failed"

  echo Stapling notary ticket to DMG
  xcrun stapler staple -v $INSTALLER_FOLDER/Selene.dmg || fail "Notary ticket stapling failed!"
fi

mv $INSTALLER_FOLDER/Selene.dmg $INSTALLER_FOLDER/Selene-$VERSION.dmg
